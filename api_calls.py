...
def gov_open_prop():
    gov = requests.get('https://api.astranaut.network/cosmos/gov/v1beta1/proposals')
...
def pool() -> float:
    pool = requests.get('https://api.astranaut.network/cosmos/distribution/v1beta1/community_pool')
...
def bonded_tokens() -> int:
    tokens = requests.get('https://api.astranaut.network/cosmos/staking/v1beta1/pool')
...
def get_inflation() -> float:
    data = requests.get('https://api.astranaut.network/cosmos/mint/v1beta1/inflation').json()
    return float(data['inflation'])


def height():
    height = requests.get('https://api.astranaut.network/blocks/latest')
    data = height.json()['block']['header']['height']
    return (data)


def supply() -> int:
    supply = requests.get('https://api.astranaut.network/cosmos/bank/v1beta1/supply')
